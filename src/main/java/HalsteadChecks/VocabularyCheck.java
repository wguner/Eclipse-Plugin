...
public class VocabularyCheck extends AbstractCheck {
	// count vocabulary
	public int count = 0;
	Set<String> uniqueVocabulary = new HashSet<String>();
	private int[] tokens = {TokenTypes.LCURLY, TokenTypes.LPAREN, 
	  ...
	  {  
		  count = uniqueVocabulary.size();
		  log(rootAST.getLineNo(),"Halstead Vocabulary: " + count);
		  //count = 0;
	  }
	  ...
	  public void visitToken(DetailAST ast) {
		  //count++;
		  uniqueVocabulary.add(ast.getText());
	  }
	  
//	  public void isAcceptableToken(DetailAST token)
//	  {
//		  int tok = token.getType();
//		  if ((contains(tok))) {
//			  count++;
//		  }
//	  }
...
//			return Arrays.stream(tokens).anyMatch(i -> i == check);
//		}
