...
public class EffortCheck extends AbstractCheck {

	public int len = 0, operator = 0, operand = 0;
  	public double vocabulary = 0, volume = 0, difficulty = 0, effort = 0;
	private Set<String> uniqueOperators = new HashSet<String>();
	private Set<String> uniqueOperands = new HashSet<String>();
	private Set<String> uniqueTokens = new HashSet<String>();
  ...
  {
	  len = operator + operand;
	  vocabulary = uniqueTokens.size();
	  volume = (int)(len * (Math.log(vocabulary) / Math.log(2)));
	  if (uniqueTokens.size() == 0) {
		  difficulty = 0;
	  } else {
		  difficulty = (uniqueOperators.size() / 2) * (operand / uniqueOperands.size());
	  }
	  effort = difficulty * volume;
	  log(rootAST.getLineNo(),"Halstead Effort: " + effort);
  }
  
  @Override
  public void visitToken(DetailAST ast) {
	  int check = ast.getType();
	  
	    if (Arrays.stream(operators).anyMatch(token -> check == token)) {
	        uniqueOperators.add(ast.getText());
	    }

	    if (Arrays.stream(operands).anyMatch(token -> check == token)) {
...
//  
//  private boolean contains(int[] operands2, int check) {
//		return Arrays.stream(operands).anyMatch(i -> i == check);
//	}
//  
//	private boolean operatorcontains(int[] operators2, int check) {
//		return Arrays.stream(operators).anyMatch(i -> i == check);
//	}
}